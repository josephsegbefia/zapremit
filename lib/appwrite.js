...
export const config = {
  ...
  ratesCollectionId: '6698db430020a81b4df6',
};
...
// Get rate, profitMargin, and offeredrate data;
export const adminProfitInfo = async () => {
  const documentId = '6698ddde001bd04e5c0e';
  try {
...
export const updateAdminRateInfo = () => {};
...
export const changeSendCountry = async (data, userId) => {
  try {
    const response = await databases.updateDocument(
      ...
      data.userId,
      {
        ...
        destinationCountryCurrencySymbol: data.destinationCountryCurrencySymbol,
        destinationCountryCode: data.destinationCountryCode,
        destinationCountryCallingCode: data.destinationCountryCallingCode,
      }
    );
    return response;
  } catch (error) {
  ...
  try {
    const transfer = await databases.createDocument(
      ...
      ID.unique(),
      {
        ...
        offeredExchangeRate: transferData.offeredExchangeRate,
        actualExchangeRate: transferData.actualExchangeRate,
        transferProfit: transferData.transferProfit,
    ...
    // CALL ZEEPAY API with transfer data or some
    return transfer;
  } catch (error) {
    throw new Error(error);
  }
